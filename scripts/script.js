...
*/
  ...
  // REMOVING THE BOTTOM MENU ON SCROLL


const bottomMenu = document.getElementById('btcont');
const homeCont = document.getElementById('hc');

// Track scrolling globally
let lastScrollTop = 0;

// Function to handle scroll behavior
function handleScroll() {
    const scrollTop = homeCont.scrollTop;
    if (scrollTop > lastScrollTop) {
        bottomMenu.classList.remove('showBottom');
        bottomMenu.classList.add('hideBottomAnime');
        setTimeout(() => {
            bottomMenu.classList.add('hideBottom');
        }, 200);
    } else {
        bottomMenu.classList.remove('hideBottom');
        bottomMenu.classList.add('showBottomAnime');
        setTimeout(() => {
            bottomMenu.classList.add('showBottom');
            bottomMenu.classList.add('showBottomAnime');
        }, 300);
    }
...
// Function to enable full screen and attach scroll behavior
function full() {
    if (bottomMenu && homeCont) {
        homeCont.addEventListener('scroll', handleScroll);
    }
...
// Function to disable full screen and remove scroll behavior
function exitFull() {
    if (bottomMenu && homeCont) {
        homeCont.removeEventListener('scroll', handleScroll); // Proper removal
    }
...
// Toggle fullscreen on checkbox change
